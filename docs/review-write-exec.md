...
- ~~**監査ログを exec した相手が消せる**~~ → **対応済み。** `--audit-log <path>` で
  リポジトリ外に出せる。起動時に実際の置き場所を表示する（既定の場所は消せることも明示）
- ~~`tokenMatches` の長さが timing で漏れる~~ → **対応済み。** SHA-256 で固定長に
  してから `timingSafeEqual` するので長さも定数時間
- ~~`showBlob` の TOCTOU~~ → **対応済み。** 先に `rev-parse` で不変の OID に解決し、
  以降はその OID だけを使う。`size` は読んだバイト数そのもの（別問い合わせをしない）。
  payload に `oid` を出してテストで検証している
- ~~checkout に 40 桁 hex を渡すと静かに detached HEAD になる~~ → **対応済み。**
  `detached: true` と警告文を返す（そのブランチは以後進まないことを伝える）
