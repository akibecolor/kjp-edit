...
# 6回目の並列レビュー（2026-08-05）

範囲 `b1874a0..HEAD`。6観点を独立に走らせ、各指摘を別のエージェントに反証させた。

🚨 **打ち切りの告知: 6観点のうち `exec-session` と `tests` は
エージェントがセッション上限で失敗し、1件も見ていない。**
反証側も transcript / auth で一部失敗した（残った指摘は全て反証を通っている）。
つまりこれは **4/6 観点の部分レビュー**で、「全部見た」ではない。

結果: **BLOCKING 0 / SERIOUS 5 / MINOR 1、反証で消えたもの 0**。全件を直した。

## 1. SERIOUS [auth] 実行の門が、門が最も効くべき構成でだけ消えていた

`--allow-exec` は「24文字以上の `--token` を明示」を要求していたが、
その検査が `if (opts.requireAuth && !opts.token) opts.token = randomBytes(32)` の**後**にあった。
`--allow-host` は requireAuth を自動でオンにするので、
**トンネルに出す構成でだけ門が無い**という最悪の形だった。

レビュアの実測: `--allow-exec --require-auth` で起動し、43文字の自動生成トークンで
`POST /api/v0/exec` が **200** を返して `PWNED-BY-AUTOGEN` を出力した。

- 門を自動生成より**前**に移した。見るのは長さではなく
  **`--token` / `--token-file` で明示したか**（`tokenExplicit`）＋長さの下限
- ⚠️ 最初に長さの検査を落としてしまい `--token short` が通った（実測して気付いた）。両方要求する
- テストに `--require-auth` / `--allow-host` の2ケースを足し、
  **明示すれば通ることも**測る（片側だけの検査にしない）
- 変異 `exec-token-gate-order`（順序を戻す）= KILLED。
  `exec-token-explicit` は順序が正しい限り冗長なので理由を書いて DEFENSIVE

**docs と README とテスト名が実装と違っていた**ので直した（このリポジトリが最も嫌う形）。

## 2. SERIOUS [ops] 読み取り用トンネルと実行で、トークンが同じ値だった
...
→ `token-read` / `token-exec` に分けた。古い `token` は **`token-read` に引き継ぐ**
（スマホのブックマークが黙って 401 にならないように）が、**実行用は新しい値**にする
＝ 漏れているかもしれない古い値では実行できない。変異 `serve-token-separate` = KILLED。

## 3. SERIOUS [ops] 「停止しました」と書いて、孫が走り続けていた

`process.kill(pid)` は Windows では TerminateProcess 相当なので、
対象の `SIGTERM` ハンドラが**走らない**。そのハンドラが `killTree()` を呼ぶ唯一の場所なので、
`--stop` では**プロセス木が一切掃除されない**（`cmd /c npm test` の中身、`claude -p` の子が残る）。
`server.mjs` に「Windows の `child.kill()` はその1プロセスしか殺さない」と自分で書いてあるのに、
運用スクリプトの停止経路がその守りを丸ごと迂回していた。

→ `taskkill /PID <pid> /T /F` で木ごと落とし、**その後もう一度数えて**
消えていなければ「まだ動いています」と出して exit 1 にする。

## 4. SERIOUS [ops] 観測していないのに不在を断言していた

`running()` は Windows 以外で `[]` を返すので、`--status` は起動中でも
「動いている kjp-edit はありません」と**断言**し、`--stop` は何も止めずに同じ文言で exit 0。
同じファイルの中で自分が「分からないなら分からないと言う」（#31）と書いているのに、
**その規則を最も破っていたのがここ**。二重起動の門も常に false になっていた。

→ `{ supported, list, why }` を返す形にし、調べられないときは
「**分からない**」と言って exit 1（手で確認する方法も出す）。
二重起動の門が効かないことも起動時に告知する。

## 5. MINOR [stdin] フィルタの例外で「停止」表示になるのに走り続けていた

`content` が配列でない行で `.filter` が TypeError を投げ、それが購読ループを抜けて
`finally { onState({running:false}) }` に落ちる。結果、**ペインは「停止」表示なのに
セッションは絶対上限まで走り続け、そのペインからは止められない**。
`transcript.mjs` は同じ形を `Array.isArray` で守っている = **この形が来ると知っていた**。

→ `Array.isArray` で守り、`feed` の呼び出しも try で囲んで購読ループを終わらせない。
1行も出さずに終わる assistant レコードも告知する（宣言どおり捨てない）。

## 6. SERIOUS [stdin] 再接続で会話が丸ごと消えていた

`chat: true` を渡すのは会話ボタンだけで、**再接続では渡していなかった**。
すると `send()` が stream-json の1行ではなく生のテキストを stdin に書き、
`claude --input-format stream-json` が**即 exit 1 で死ぬ**
（レビュアの実測: 走っている会話セッションに生の行を1本書くだけで
`CLOSE code=1` / `Error parsing streaming input line`）。
keepAlive なので猶予では死なないのに、**入力形式の取り違えで文脈が丸ごと消える**。

→ 判定を `isChatArgv(argv)` として `v0/argv.mjs` に切り出し（**app.html の中では
テストできない = 宣言が破れても気付けない**、#44 と同じ理由）、
再接続でも `s.argv` から復元する。変異 `chat-argv-detect` = KILLED。

## この回で学んだこと

- **順序が守りの本体になっている場所がある。** #1 は「検査の内容」ではなく
  「検査の位置」が穴だった。**自動生成やフォールバックより前に門を置く**
- **同じ資格情報を2つの capability で使い回さない。** #2 は Cookie の件と同型で、
  「危険度が桁違いなものをまとめない」という分界が**資格情報のレベルで**崩れていた
- **「調べられない」と「無い」を区別する。** #4 は自分が書いた規則の違反。
  規則を書いても、**書いた場所から遠いコードには適用し忘れる**
- **レビューは打ち切られうる。** 2観点が丸ごと失敗した。
  「反証で消えたもの 0」は「全部本物」であって「全部見た」ではない

---

# 7回目の並列レビュー（2026-08-05〜06）

範囲 `b1874a0..HEAD`。**全6観点、エージェント失敗0**（前回は2観点が上限で失敗した）。
結果: **BLOCKING 0 / SERIOUS 13 / MINOR 2、反証で消えたもの 1**。全件を直した。

⚠️ 経緯: 抜けていた2観点だけを走らせるつもりで `args.only` を足したが、
resume が**全6観点のキャッシュを再生**したので全観点の結果が揃った。

## 特に重い2件（read が RCE に昇格する / 痕跡ゼロで総当たり）

### コマンド行に載った実行トークンが read 権限で読めた

「Cookie は他ポートに漏れるが、**漏れても読み取りまで**」という分界が、
**資格情報がコマンド行に載ることで崩れていた**。README が案内していた起動手順が
`--allow-exec --token "$TOKEN"` で、リテラルで打った回は記録に残る
（実データで 42 件）。`/api/v0/state` は Cookie だけで 200 を返すので、
読み取り専用の相手が実行トークンを回収できた。

→ `maskSecrets()`（値 + キーワードの直後）+ 告知 + README を `--token-file` 側へ。
**完全な防御ではない**ことを文書に明記した。

### 読み取り/書き込みトークンに下限も記録も遅延も無かった

下限は exec 経路にしかなく `--token abc` が通り、401 はどこにも残らず、
遅延も回数制限も無い。レビュアの実測で **29回目に 200**、17,576 回外しても絞られない。

→ 下限を capability 共通に。失敗を監査に（**本文は残さない**）。指数の遅延（上限 2s）。
実測で 40 回が 67 秒（1回 1685ms）になった。

## 「規則を書いた場所から遠いコードには適用し忘れる」が3回出た

| 規則 | 破っていた場所 |
|---|---|
| 「停止しました」は数え直してから書く | `killTree` が taskkill の終了コードを捨てていた（規則は `serve.mjs` にだけ適用されていた） |
| 分からないなら分からないと言う | `--status` が `--allow-host` を出さない（tailnet 全体から読めるデーモンとループバック専用が同じ1行） |
| 打ったフラグを黙って捨てない | 二重起動の判定が capability の差を黙って捨て、**RCE 可能なデーモンの URL を案内**していた |

## 順序が守りの本体だった（2件目）

`/kill` と sweeper が **finish → killTree** の順だったので、
(a) 実際に死ぬまでの出力が live に届かず告知も無い
(b) 再購読すると `exit` の後ろに out が並ぶ
(c) 出力が多いと `exit` 自身がリングから押し出されて終端が消える。

→ **殺してから終端する**に変えた。⚠️ **その変更が停止理由を消す回帰を作り、
既存テストが捕まえた**（子の 'exit' が先に終端して `code:1` になり、finish に
載せた「上限時間を超えたので停止します」が捨てられた）。理由は殺す前に流す形にした。

## 日本語入力で誤実行していた（IME）

`isComposing` の参照がリポジトリ全体に1つも無く、**変換確定の Enter で
半端な argv がそのまま実行**されていた（標準入力側は未変換の読みを送っていた）。
字面の検査では意味が無いので、`render-check.mjs` で**合成イベントを撃って
副作用の有無を測る**形にした。

## この回で学んだこと

- **「守りがあるのに検査が1件も無い」を探すと出てくる。** `--no-textconv` は
  `core.fsmonitor` と**同じコミット**で入ったのに、fsmonitor だけ検査があった。
  同じ日に入れた守りでも、片方だけ測っている
- **`-z` の多トークンは再発トップの罠と自分で書いているのに、`git mv` が
  テストに1回も無かった。** 「罠だと書く」ことと「測る」ことは別
- **検証コマンド自身が「打ち切ったら告知する」を破っていた。**
  先行ステップが落ちると smoke / layout / render の**行が消えて**、
  5つのうち3つが走っていないことがどこにも出なかった
- **測れないものは「測れない理由」を書く。** `killTree` の数え直しは
  「落ちないプロセスを移植可能に作れない」ので DEFENSIVE にした。
  一方 `--no-ext-diff` は「測れない」と書きかけて**実測したら落ちた** —
  推測でコメントを書かないこと
- **規則8（バックスラッシュが層をまたいで落ちる）を2回踏んだ。**
  1回は `\d+` が `d+` になって**対照の検査が偽陰性**になった（起動しているのに
  検出できない形）。バックスラッシュを使わない書き方（`[0-9]+`）に逃げるのが確実
