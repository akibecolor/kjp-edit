...
🚨 **同じ告知の連続はまとめます。** `thinking` / `stream_event` は**トークンごとに
1レコード**来るので、「表示していません」を1件1行で出すと**画面が告知で埋まり、
肝心の応答が押し出されます**（スマホの実画面の8割がこれになりました。
実測では56行のうち24行が `system/thinking_tokens`）。
最初の1件を出し、続く同じ告知は `（同上 ×23 を省略）` として**件数を必ず見せます**
（黙って捨てない約束は守る）。応答本文とツール名は待たせません。
