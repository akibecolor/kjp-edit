...
| 失効したら | 🔒 **実行トークンが回転します**（承認した端末は `cat token-exec` で生トークンを写せるので、台帳から外すだけでは写しが生き続ける）。写された生トークンは死に、**他の承認済み端末はそのまま使えます**。⚠️ 生トークンを貼ったタブと読み取り Cookie は無効になるので `?token=` 付き URL を開き直します |
