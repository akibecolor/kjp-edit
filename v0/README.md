...
### 🔒 複数リポジトリ（読める範囲を広げられるのは exec 級だけ）
...
🚨 **読み取り・書き込みの鍵では、範囲を後から広げられません。**
「開きたいものを入力する」形を**読み取りの鍵に許すと**、トークンが1本漏れた時点で
**マシン上の全 git リポジトリが読める**状態に化けます。
だからクエリの `?repo=` は**登録済み一覧との `samePath()` 照合**しか通しません
（`isSafeRepoPath()` ではなく登録との照合。**形式が正しくても登録外は 400**）。

⚠️ **ただし `--allow-exec` のときだけ「開く」で足せます**（下の「プロジェクトを開く」）。
根拠は、exec 級の相手は `argv=['cat','<任意のパス>']` で**すでに任意のファイルを読める**ので、
登録一覧に足すことが**新しい権限にならない**こと。読み取り／書き込みの鍵には許しません。
つまり正確には「**読める範囲は起動時に固定される。広げられるのは exec 級だけ**」です。
...
| **未コミットの変更**（#77） | タブに **`*名前`** で出る。選ぶと **HEAD ↔ 作業ツリー**の差分（= **エージェントが今書いたもの**）。エージェントは作業中ほとんどコミットしないので、`base...HEAD` だけでは一覧に出なかった |
| **未追跡ファイル**（#77） | タブに **`+名前`** で出る。選ぶと**編集画面に入らず読める**（スマホの誤タッチで内容が変わらないように）。`--allow-untracked` が要る |
...
| **エージェントの活動**（`--watch-agents`） | 稼働中 / 待機 / 記録なし、直近のツールと触ったパス、件数。**既定オフ**（リポジトリ外の記録を読むため）。🔒 発話とコマンド行（`--allow-transcript-text`）は**実行の鍵を貼ったタブにだけ**返す（B4。走っている実行の argv と同じ扱い） |

**ペインは自由に動かせます。** ヘッダを掴んで並べ替え、列（左 / 差分 / コンソール / 右）を
またいで移動できます（マウスとタッチの両方。スマホでも掴めます）。
**既定は格子配置（グリッド）です**（#75。2026-08-09 に切り替え）。
パターンは 1×1 / 1×2 / 1×3 / 2×2 / 2×3 / 3×3 / 4×4 で、
セルの**結合・分割・閉じる・開き直し**ができます。
配置ボタンは**実際の分割を描いたアイコン**です（#76。以前は `1×1 1×2 …` という
文字の羅列だった）。意味は `title` と `aria-label` に残してあるので読み上げでも分かります。
検査（layout-check）は**大きさが揃っていること**に加えて、
**アイコンが行×列を実際に描けていること**（セル数・列数・占有面積）も実ブラウザで測ります。
旧レイアウト（固定4カラム）は **`?grid=0`** で残してあります。
⚠️ 消していないのは、**入れ物をまたぐドラッグの検査が旧レイアウトを見ている**ためです。
検査は両方測っています — 既定は幅ごとの溢れ・隠れ・配置ボタン（layout-check）と
セル移動・× の実クリック（input-check）、旧レイアウトは並べ替えと復元（layout-check / render-check）。
狭い画面で PC レイアウトを見たいときは **`?wide=1`**（スマホの横画面用）。

**配色は自動 / ライト / ダークから選べます**（トップバーの「配色」ボタン）。
既定は**ダーク**です。**自動**を選ぶと OS の設定に従います（切り替わったら追従）。
選んだ値は `localStorage` に残ります。
⚠️ **「色を用意した」と「読める」は別**なので、両方の配色で
**実ブラウザに描かれている色**のコントラスト比を測っています（`node v0/theme-check.mjs`）。
ライトはダークの色を流用していません — 流用すると白地で警告色が 2.31、成功色が 2.04 になり、
WCAG の大きい文字の基準（3.0）すら下回ります（実測）。
...
| `theme.mjs` / `.test.mjs` | 配色の判定（自動 / ライト / ダーク）とコントラスト比。**同じ理由で共有** |
| `theme-check.mjs` | 実ブラウザで**実際に描かれている色**を読み、両方の配色で読めることを測る |
| `cdp.mjs` | 実ブラウザ検査の定型（Chrome 起動 → CDP 接続 → 評価 → 後始末）。input と theme が共有 |
...
| 払い出す量 | 最後の出力は1セッション最大 **8000 文字**。⚠️ **実測で決めた値**です — claude の stream-json は行長の中央値 420 文字ですが `result` 行は**最終回答の本文を含むので 7813 文字**あり、2000 文字で切ると「応答おわり」が必ず壊れて監視盤に `…"cache_read_input_tokens":42857` のような断片が出ました（実機で踏んだ）。⚠️ **8本で 64KB という見積もりは ASCII 前提でした**（MINOR。10回目のレビュー）。日本語の応答では1文字3バイトなので、実測で**1回あたり最大 194KB / 47KB/s** です。上限は文字数で縛っているので、バイト数はこの3倍を見込んでください |
...
| 終了後の保持 | 既定 600秒**かつ 200 件**。過ぎたら台帳から消えます（#72） |
...
🚨 **「停止しました」が保証する範囲（#47。過大に主張しないための告知）。**

| | |
|---|---|
| 保証する | 撃つ**時点で木に載っていた** pid を撃った後に数え直します。1本でも生きていれば `{ok:true}` を返さず、「木の一部を確認できませんでした（pid …）」を UI と監査に出します |
| 保証しない | 撃つ時点で**中間プロセスが既に終了している**孫。中間が消えると孫の親 pid は死んだ pid（POSIX では init）を指すので、**自分の子孫だったことを親子関係から復元できません**。`taskkill /T` も `kill(-pgid)` も届かず、数え直しにも掛かりません（実測で再現） |
| 起こる条件 | 中間が孫を `detached` で起こして自分が先に終わる形。`claude` や `npm test` のような常用の形では中間が生きているので**今の数え直しで捕まります** |
| 直すなら | Job Object（Windows）/ cgroup（Linux）で囲むのが本来の解ですが Node 標準からは作れません。木を数秒おきに蓄積する案は Windows で1回あたり数百 ms かかるので、依存ゼロの観測ツールとして代償が大きい（issue #47 に実測つきで記録） |
...
エージェントが作った**まだ add していない新規ファイル**を画面から直せます
（利用者の要件 2026-08-18）。ファイラのタブに **`+ファイル名`** で出ます。

```bash
node scripts/serve.mjs --exec               # 自動で有効（下の理由）
node scripts/serve.mjs --write              # 追跡ファイルのみ（今までどおり）
node scripts/serve.mjs --write --untracked  # 明示的に有効化
```

| | |
|---|---|
| 対象 | **未追跡かつ gitignore されていない**ファイルだけ（`git ls-files --others --exclude-standard`） |
| 🔒 対象外 | **gitignore されたファイル（`.env` など）** / worktree の外 / シンボリックリンク / バイナリ |
| なぜ `--exec` で自動か | 実行できる相手は既に `cat > file` で未追跡ファイルを書けるので、**権限が1つも増えない**（「プロジェクトを開く」と同じ論理）。`--write` だけの構成では**黙って範囲を広げない**ため明示を要求します |
| 表示 | ビューア（差分・全文）は `git cat-file` 経由なので**未追跡は読めません**。選ぶと理由が出て、「編集」で中身が開きます |

⚠️ **まるごと新しいディレクトリは一覧に出ません**（`git status` が `dir/` に畳むため。
数を増やさないために `--untracked-files=normal` のまま使っています）。
その中のファイルを直すには、先に `git add` するか端末で編集してください。

⚠️ これは **#11（`.env` を人が触る）とは別の需要**です。#11 は ignored が対象なので開いたままです。

### 📁 プロジェクトを開く（登録一覧にリポジトリを足す）

トップバーの「**開く**」からローカルのリポジトリのパスを打つと、登録一覧に足されて
セレクトで切り替えられます（設計は [../docs/open-project.md](../docs/open-project.md)）。

```bash
# 控えを渡して起動すると、開いた分が再起動後も残る（scripts/serve.mjs は自動で渡します）
node v0/server.mjs --allow-exec --token-file ~/.kjp-edit/token-exec \
                   --repos-file ~/.kjp-edit/repos.json
```

| | |
|---|---|
| 門 | 🔒 **`gateExec`**（`--allow-exec` + 生の実行トークン or 承認した端末の鍵） |
| なぜ合言葉が要らないか | exec 級の相手は `argv=['cat','<任意のパス>']` で**すでに任意のファイルを読める**ので、登録一覧に足すことは**新しい権限を与えない**（単なる利便性）。逆に読み取り／書き込みの鍵に許すと**範囲の昇格**になるので閉じています |
| 何を受け付けるか | git リポジトリのルート（bare も可）。**linked worktree を渡すとリポジトリ側に解決**します |
| 外す | 足した分は外せます。⚠️ **起動時に `--repo` で渡した分は HTTP から外せません**（起動の意図を覆さないため） |
| 記録 | `repo-add` / `repo-remove` を監査に残します |

⚠️ **ファイルの一覧・参照（ピッカー）は作っていません。** 作ると「HTTP から
ファイルシステムを歩ける」経路になり、exec とは別クラスの攻撃面が増えるためです（パスは打つ）。
...
| 失効したら | 🔒 **実行トークンが回転します**（承認端末は `cat token-exec` で写せるため）。写された生トークンは死に、**他の承認済み端末とフックはそのまま**使えます（読み取り秘密の配布先も配り直します）。⚠️ 貼ったタブと古い Cookie は無効。🚨 **exec で仕込まれた永続化（cron / 自動起動 / worktree 内のスクリプト）は回転では消えません** — 失効は「漏れた鍵の無効化」で、完全な封じ込めではありません |
...
| 開いた記録 | ページを開くと `page-open` を追記します（**いつ・どこから・どの鍵で**だけ。URL も自由文も残しません）。⚠️ **状態の自動更新では記録しません** — 15 秒ごとなので 1 日 5,760 件になり、他の記録が回転で消えます。同じ出所は 60 秒に 1 回に絞り、**絞った件数は次の記録の `suppressed` に載せます**（捨てません）。読み出しは `node scripts/usage.mjs` |
...
つまり**リポジトリ外のファイルも、`.env`（gitignore 済み）も読めません**
（スモークテストで固定してあります）。

⚠️ **範囲が1つ広がりました（#77。2026-08-18）。** `diff` に `mode=worktree` を足したので、
読み取りの鍵でも **HEAD ↔ 作業ツリーの差分（追跡ファイル）** が読めます。
「**エージェントが今何を書いたか**」がこのツールの中心的な価値なのと、読み取りの鍵は
すでにリポジトリの全履歴を読めるため（判断の経緯は `../docs/editor-filer.md` §4）。
🔒 **ignored も未追跡も含まれません**（`git diff` は追跡ファイルだけを見る）。
対象の worktree は**登録済み一覧と照合**します。
⚠️ **未追跡ファイルの中身**は今も `diff`/`blob` からは読めません
（読めるのは編集と同じ門を通る `--allow-untracked` の相手だけ）。
