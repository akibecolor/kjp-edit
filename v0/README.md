...
| 🚨 誰が動かしたか | **トンネル越しは全部 `127.0.0.1` に見えます**（`tailscale serve` がこのマシンで終端して中継するため。実データで確認）。中継が付けた `x-forwarded-for` は `xffReported` として残しますが、**自己申告なので認可には使えません**（ループバックに届く相手なら誰でも書ける）。`peer` を上書きしないことをテストで固定しています |
