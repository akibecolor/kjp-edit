...
export class ExecRegistry {
    ...
    /**
     * 🚨 **「触っている = 見ている」を寿命の判断に入れる（#49）。**
     *
     * 監視盤（`/api/v0/exec/list` + `/input`）は**購読せずに**セッションへ
     * 入力できる。一方で切断後の猶予は購読/解除の時刻でしか進まなかったので、
     * **スマホから返事を書いている最中に「切断されたまま N 秒経った」で
     * SIGKILL される**（並列でエージェントを回す本来の使い方でだけ起きる）。
     * 入力が通ったら猶予の起点を今にする。
     *
     * ⚠️ **一覧の取得では呼ばないこと。** 監視盤は数秒ごとに**全件**を見るので、
     *    そこで延ばすと猶予が実質無効になる（タブを開いているだけで
     *    全セッションが絶対上限まで生き残る）。延ばすのは
     *    **その1本に向けた操作**（入力）だけ。
     * @returns {boolean} 猶予を延ばしたか（購読中なら既に猶予の外なので false）
     */
    noteInput(s) {
        if (!s?.running) return false;
        // 購読者が居る間は `lastDetachedAt === null`（猶予の外）なので触らない
        if (s.subscribers.size) return false;
        s.lastDetachedAt = this.now();
        return true;
    }
